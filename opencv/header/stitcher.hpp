...
public:
    ...
    Stitcher(std::vector<cv::Mat>& images, std::vector<cv::Mat> &grayscaledImages, cv::Mat &currentStitchedImage,  std::vector<std::vector<cv::KeyPoint>> &imagesKeypoints,std::vector<cv::Mat> &currentImageDescriptor,cv::Ptr<cv::Feature2D>& orb);
    void processImage();
